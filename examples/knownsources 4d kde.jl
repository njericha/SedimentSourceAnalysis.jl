...
# take all measuraments
selected_measurements = getmeasurements(grain1)[2:5] #["Eu_anomaly","Ti_temp", "Dy_Yb"]
sinks = [sink[selected_measurements] for sink in sinks]::Vector{Sink}
sink1 = sinks[1]

n_measurements = length(selected_measurements)
...
var_names = ["x$i" for i in 1:n_measurements]

makekde(sink_measurements) = fastkde.pdf(sink_measurements...,axes = coords,do_xarray_subset=false, var_names = var_names, num_points = M).values |> reversedims

PDF = fastkde.pdf(sink1_measurements...,axes = coords,do_xarray_subset=false, var_names = var_names, num_points = M)
#axes = coords
...
x_coord, y_coord, z_coord = coords[1:3]
...
domains = coords#[x_coord, y_coord, z_coord]

# Find the best rank and perform the nonnegative decomposition Y=CF
Y = cat(densities..., dims=n_measurements+1)
Y = permutedims(Y, [n_measurements+1,(1:n_measurements)...])
...
factortensor_true = cat(true_densities..., dims=n_measurements+1)
factortensor_true = permutedims(factortensor_true, [n_measurements+1,(1:n_measurements)...])
...
match_sourcesNd!(C, F, coefficientmatrix_true, factortensor_true)
