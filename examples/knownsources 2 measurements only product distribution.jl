...
measurement1 = selected_measurements[1]
println("densitytensor $measurement1 density (lateral slice)")
display(densitytensor[:, measurement1, 1:1]) #just the first 1 sample
...
p = plot_densities(densitytensor, measurement1);
plot!(xlabel=measurement1,
...
raw_data_ages_sink1 = [grain[measurement1] for grain in sink1]
KDE_ages_sink1 = densitytensor[1, measurement1, :] #sink 1
...
plot!(getdomain(densitytensor, measurement1), KDE_ages_sink1;
    label="KDE",
    color=:blue,
    linewidth=5,
    alpha=1
...
p = plot(getdomain(densitytensor, measurement1), KDE_ages_sink1;
label="continuous KDE",
color=:blue,
xlabel="age (millions of years)",
ylabel="probability density"
)
scatter!(getdomain(densitytensor, measurement1), KDE_ages_sink1;
marker=:circ,
markercolor=:blue,
...
scatter!(getdomain(densitytensor, measurement1)[1:4:end], KDE_ages_sink1[1:4:end];
marker=:circ,
markercolor=:blue,
label="KDE discretization")
display(p)
...
x_coarse=getdomain(densitytensor, measurement1)[1:step:end]
