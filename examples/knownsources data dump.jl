...
# the default for BlockTensorDecomposition.DensityEstimationTools.DEFAULT_N_SAMPLES is 64
n_samples = 2^8 + 1
...
densities, domains = standardize_KDEs(raw_densities;n_samples) #Made it to here so far
...
Y_257_points = copy(array(densitytensor)); # plain Array{T, 3} type for faster factorization

Y_lateral_slices = eachslice(Y_257_points, dims=2)
...
# Find the best rank and perform the nonnegative decomposition Y=CF
Y_1025_points = copy(array(densitytensor)); # plain Array{T, 3} type for faster factorization

Y_lateral_slices = eachslice(Y_1025_points, dims=2)
Y_lateral_slices .*= getstepsizes(densitytensor)

using HDF5

myfile = h5open("./geodata.h5", "w")

#create_group(myfile, "mygroup")
myfile["Y_257_points"] = Y_257_points
myfile["Y_1025_points"] = Y_1025_points
